...
6. Wyjątek: jeśli właściciel repozytorium **wprost mówi**, że chodzi tylko o jednorazową analizę /
   wyjaśnienie bez zmian w repo (zgodnie z głównym `AGENTS.md`), możesz potraktować ten plik
...
- `tasks/` – plany operacyjne.
  Każde zadanie ma folder `<ID-Z>_<status>/` ([możliwe statusy](#files-statuses)) z:
    ...
    - `tasks.md` – plan wykonawczy rozbijający cel na kroki i pod‑kroki. Zawiera kolumny:
      ...
      - Procedura audytu template'ow `.j2` (zakres + subagenci + Claude + Gemini) jest opisana w `agents-tasks-knowledge/tasks/AGENTS.md`, sekcja „3.4. Audyt template'ow `.j2` (zakres i procedura)”.
...
0. Jeżeli w pierwszej wiadomości w tej rozmowie użytkownik **prosi o założenie nowego zadania w `tasks/`**
   (np. „załóżmy nowe zadanie dla …”, „pomóż mi założyć nowe zadanie w `tasks/`”, „chcę nowy katalog zadania”):
    ...
    - potraktuj tę prośbę jako wejście w „happy path” z `agents-tasks-knowledge/tasks/AGENTS.md`, sekcja „9. Flow właściciel ↔ agent”,
    ...
    - sam plik `agents-tasks-knowledge/tasks/AGENTS.md` traktuj jako referencję w tle, a nie coś, co użytkownik musi czytać, żeby ruszyć dalej.


1. Jeśli w tej rozmowie użytkownik **wprost zaznaczył**, że chodzi tylko o jednorazową analizę /
   wyjaśnienie bez zmian w repo (patrz TL;DR i główny `AGENTS.md`):
...
3. Stwórz plan działania małymi krokami i **interaktywnie** ustalaj szczegóły z właścicielem zadania:
    ...
    * jeżeli plan tworzysz od zera, a w `tasks.md` nie ma jeszcze wiersza `ID-T = 01` opisującego
    ...
    > `agents-tasks-knowledge/tasks/AGENTS.md`, sekcja „3.3. Tryb „aktywnego dopytywania” przez Agenta”,
...
9. Podczas realizacji zadania:
    ...
    * uruchamiaj odpowiednie testy przez `make` (szczegóły w lokalnych `AGENTS.md` dla komponentów, jeśli istnieją); jeśli test nie istnieje, zaproponuj jego dopisanie,
...
(patrz `agents-tasks-knowledge/tasks/AGENTS.md`, sekcja „4.2. Jak numerować `ID-T`”).
...
* Przykład jest w pełni spójny z zasadą `ID-T = 01` z pliku `tasks/AGENTS.md`.
