# Plan zadań <ID-Z>_proposal - <krótki tytuł> (szablon do skopiowania)

> Szybki start: skopiuj katalog `template-task_proposal/` do `tasks/<ID-Z>_proposal/`, zmień nagłówek/tabelę, a w `agents-tasks-knowledge/SESSION*.md` ustaw `current-task = <ID-Z>_proposal`, `current-id-t = 01`.

<!-- UWAGA: Po skopiowaniu zmień daty YYYY-MM-DD HH:MM na rzeczywiste! -->

<!-- audit-gate:plan-v1 -->

| ID-T | Status   | Agent | Rodzic | Zadanie                                                               | Opis                                                                                                  | Utworzono        | Zaktualizowano   | Co zrobiono do tej pory                                                                                                    |
|------|----------|-------|--------|-----------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------|------------------|------------------|----------------------------------------------------------------------------------------------------------------------------|
| 01   | planning | ba    |        | Doprecyzowanie i uzupełnienie `additional-contexts.md` + przygotowanie planu technicznego | Uporządkuj sekcje 1-8 w `additional-contexts.md`, dodaj otwarte pytania w sekcji 6, zaproponuj kolejne ID-T i statusy. | YYYY-MM-DD HH:MM | YYYY-MM-DD HH:MM | Start: zweryfikuj źródła w sekcji 1, wpisz pytania w sekcji 6 i przygotuj szkic planu w kolejnych wierszach `tasks.md`. |
...
- [ ] Zmieniono daty `YYYY-MM-DD HH:MM` na rzeczywiste w tej tabeli
