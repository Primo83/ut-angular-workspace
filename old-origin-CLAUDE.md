...
2. **Decyzja: ad‑hoc vs. system zadań**
    ...
    - w pozostałych przypadkach **domyślnie** korzystasz z systemu zadań: `agents-tasks-knowledge/CLAUDE.md`, `SESSION*.md`, `tasks/`.
...
1. Agent buduje łańcuch instrukcji z plików user-owned (`CLAUDE.md`, a dla innych klientów odpowiednio `AGENTS.md` / `GEMINI.md`):
    - globalne (`~/.Agent/CLAUDE.md`),
...
2. Na poziomie **tego repozytorium** przyjmij następującą logikę:
    ...
    2. Ten plik `CLAUDE.md` w katalogu głównym repo – ogólne zasady, bezpieczeństwo, proces zadań.
    3. `agents-tasks-knowledge/CLAUDE.md` – definicje statusów, ogólne zasady pracy na `tasks/` i `ideas/`.
    ...
    7. **Priorytet numeryczny w razie konfliktu (od najwyższego do najniższego):**
       ...
       7. Globalne `~/.Agent/CLAUDE.md` (poza kontrolą projektu, najniższy priorytet).
...
0. Jeśli jako właściciel chcesz **założyć nowe zadanie od zera** i być prowadzony „za rękę”, zacznij od pliku
   `agents-tasks-knowledge/tasks/CLAUDE.md`, sekcje:
   ...
   **Nie odsyłaj go tylko do pliku.** Zamiast tego, krok po kroku:
   ...
   - dopiero na końcu, jeśli chce szczegóły, wskaż odpowiednie sekcje `agents-tasks-knowledge/tasks/CLAUDE.md` jako referencję.
...
2. Szczegółowe zasady i definicje statusów (`proposal`, `to-do`, `planning`, `planned`, `in-progress`, `on-hold`, `done`) znajdziesz w:
    - `agents-tasks-knowledge/CLAUDE.md` (sekcja „Files statuses”),
    - `agents-tasks-knowledge/tasks/CLAUDE.md` (proces tworzenia nowych zadań).
3. Struktura i sposób użycia `additional-contexts.md` (w tym szablon BA, tryb „aktywnego dopytywania”, w którym właściciel wypełnia głównie sekcję
   ...
   `agents-tasks-knowledge/tasks/CLAUDE.md`, sekcja „3. Wypełnianie `additional-contexts.md`
...
9. Jeśli właściciel repozytorium prosi tylko o jednorazową analizę / wyjaśnienie **albo o przykładowy kod „na tablicy”, który nie jest proponowany jako bezpośrednia zmiana plików w repo**:
   ...
   - jeśli z analizy wynika potrzeba dalszych prac – zaproponuj utworzenie zadania zgodnie z `agents-tasks-knowledge/tasks/CLAUDE.md`.
...
10. System zadań jest przygotowany na pracę wielu agentów równolegle:
    ...
    - szczegółowe zasady są opisane w `agents-tasks-knowledge/CLAUDE.md`, sekcja „Multi‑agent (AGENT_ID i kolumna `Agent`)” oraz „Multi‑agent: `AGENT_ID` i pliki `SESSION_<AGENT_ID>.md`”.
...
>`agents-tasks-knowledge/CLAUDE.md`.
